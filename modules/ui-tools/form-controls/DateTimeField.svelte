...
<main>
	<input type="datetime-local" value={formatDate(value)} oninput={onInput}/>
	<button use:tooltip={"Unset"} class="reset" onclick={resetValue}><i class={Icon.solid("times").with(Icon.FW).class}></i></button>
	<button use:tooltip={"Now"} class="date-now" onclick={setCurrentDate}><i class={Icon.regular("calendar-clock").with(Icon.FW).class}></i></button>
</main>
